                            ...
                            <div class="table-responsive mt-3">
                                <table id="datatable-table" class="table text-center text-nowrap">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{ $no++ }} </td>
                                            <td>{{ $g->gelombang->gelombang }} </td>
                                            <td>{{ $g->pelatihans->nama_pelatihan ?? '-' }} </td>
                                            <td>{{ $g->jumlah_peserta }} </td>
                                            <td>{{ $g->gelombang->tanggal}} -
                                                {{ date('H:i', strtotime($g->gelombang->jam)) }} </td>
                                            <td>
                                                @if($g -> ujian)
                                                {{ $g->ujian->nama }}
                                                @else
                                            ...
                                            <td>
                                                <a href="javascript:void(0)" data-toggle="modal"
                                                    data-target="#edit_instruktur" data-instruktur="{{ $g->id }}"
                                        ...
                                        @else
                                        <tr>
                                            <td colspan="7" class="text-center">Silakan pilih gelombang untuk menampilkan data.</td>
                                        </tr>
                                        @endif
                                    </tbody>
                                </table>
            ...
            <form action="{{ url('/admin/edit_gelombang') }}" method="POST">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="edit_instrukturLabel">Edit Gelombang</h5>
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <div class="row">
                                ...
                                <div class="col-6">
                                    <input type="time" name="jam" id="jam" value="{{ old('jam') }}" class="form-control"
                                        required>
                                </div>
                            </div>
                        </div>
